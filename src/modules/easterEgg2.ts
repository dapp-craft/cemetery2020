...
export function addEasterEgg() {
    ...
    easterEgg.addComponent(new OnPointerDown(
        () => {
            ...
            rewardDummy.addComponent(new Transform({ position: new Vector3(62.7, 0, 47), scale: new Vector3(2, 2, 2) }))
            engine.addEntity(rewardDummy)
            const reward = new Reward(rewardDummy, 'egg2', { position: new Vector3(0, 1, 0) }, true, () => {
                executeTask(async () => {
